...
get_tool_description() {
    case "$1" in
        ...
        sessions) echo "Persistent SSH sessions with state, replay, and inferred memory" ;;
        ...
        gamechanger) echo "cat/edit/diff, systemctl/journalctl/docker wrappers, port probe, tail sessions, artifact deploy, multi-step plans" ;;
        *) echo "" ;;
    esac
}
...
cmd_tools_list() {
    ...
    if [ ! -f "$TOOLS_CONFIG" ]; then
        ...
        echo ""
        echo "${GRAY}Default: All 51 tools enabled${NC}"
        echo ""
    ...
    case "$mode" in
        ...
        custom)
            ...
            for group in core sessions monitoring backup database advanced gamechanger; do
...
cmd_tools_reset() {
    if [ -f "$TOOLS_CONFIG" ]; then
        print_warning "This will delete your tool configuration and enable all 51 tools"
        echo ""
...
cmd_tools_configure() {
    ...
    echo ""
    echo "claude-code-ssh has ${BOLD}51 tools${NC} organized into ${BOLD}7 groups${NC}:"
    echo ""
...
EOF
            echo ""
            print_success "Configuration saved: ${GREEN}All tools mode${NC} (51 tools)"
            ;;
    esac
    ...
    case "$mode" in
        all)
            ...
            tools=("ssh_list_servers" "ssh_execute" "ssh_upload" "ssh_download" "ssh_sync"
                   ...
                   "ssh_session_replay" "ssh_session_memory"
                   ...
                   "ssh_cat" "ssh_edit" "ssh_diff" "ssh_systemctl" "ssh_journalctl" "ssh_docker"
                   "ssh_port_test" "ssh_tail_start" "ssh_tail_read" "ssh_tail_stop"
                   "ssh_deploy_artifact" "ssh_plan")
            ;;
        minimal)
            tools=("ssh_list_servers" "ssh_execute" "ssh_upload" "ssh_download" "ssh_sync")
            ;;
        custom)
            ...
            if [ "$(jq -r '.groups.gamechanger.enabled // false' "$TOOLS_CONFIG")" = "true" ]; then
                tools+=("ssh_cat" "ssh_edit" "ssh_diff" "ssh_systemctl" "ssh_journalctl" "ssh_docker"
                        "ssh_port_test" "ssh_tail_start" "ssh_tail_read" "ssh_tail_stop"
                        "ssh_deploy_artifact" "ssh_plan")
            fi
            ;;
